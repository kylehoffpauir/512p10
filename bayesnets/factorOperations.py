...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        # Initialize a new Factor with the appropriate unconditioned
        # and conditioned variables.  The set of unconditioned
        # variables is the original set with the elimination variable
        # removed.  However, do not use remove(), since this would
        # affect the original set.  Also note that the value of the
        # elimination variable is a string, so do not create a set
        # containing this variable using set().
        ...
        # Fetch the domain for the elimination variable once.
        eliminationVariableDomain = factor.variableDomainsDict()[eliminationVariable]
        # For each assignment in the new Factor, sum the original
        # Factor over corresponding assignments that include each
        # value of the elimination variable.  Store each sum as
        # the probability value in the new factor.
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            sum = 0.0
            factorAssignment = assignment.copy()
            for value in eliminationVariableDomain:
                factorAssignment[eliminationVariable] = value
                sum += factor.getProbability(factorAssignment)
...
def normalize(factor):
    ...
    # First, compute total probability and check it for 0
    total = 0.0
    ...
    # Create the new Factor with appropriate conditioned/unconditioned
    # variables
    conditionedVariables = factor.conditionedVariables()
    unconditionedVariables = set()
    ...
    # Copy probabilities from original factor to new factor, normalizing
    # as we go.
    for assignment in factor.getAllPossibleAssignmentDicts():
        newFactor.setProbability(assignment,
                                 factor.getProbability(assignment)/total)
