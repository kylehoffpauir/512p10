...
def constructBayesNet(gameState):
    ...
    edges.extend([(X_POS_VAR, house) for house in HOUSE_VARS])
    edges.extend([(Y_POS_VAR, house) for house in HOUSE_VARS])
    edges.extend([(house, obs) for house in HOUSE_VARS for obs in obsVars])
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    factor = inference.inferenceByVariableElimination(bayesNet,
                                                      FOOD_HOUSE_VAR,
                                                      evidence,
                                                      eliminationOrder)
    assignment_with_best_house_val = max([
        (assignment, factor.getProbability(assignment))
        for assignment in factor.getAllPossibleAssignmentDicts()],
                               key=lambda p : p[1])[0]

    # Following directions at web page, which say to return a dictionary
    # with a single entry
    #  {FOOD_HOUSE_VAR: best_house_val}
    # However, returning the full assignment containing this key/value pair
    # should also work.
    return {FOOD_HOUSE_VAR: assignment_with_best_house_val[FOOD_HOUSE_VAR]}
