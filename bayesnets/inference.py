...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # Obtain the initial list of factors, instantiated with evidence
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Perform the main loop: For each hidden variable (in
        # elimination order), form the pointwise produce of factors
        # having this variable and then eliminate the variable from
        # the resulting factor.
        for eliminationVariable in eliminationOrder:
            (currentFactorsList, resultingFactor) = joinFactorsByVariable(
                currentFactorsList, eliminationVariable)
            if len(resultingFactor.unconditionedVariables()) > 1:
                currentFactorsList.append(
                    eliminate(resultingFactor, eliminationVariable))

        # Join (compute pointwise produce of) all remaining factors
        # and normalize
